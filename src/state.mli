...
(** [initalize_players_hands pl deck] distributes five cards to each player in
    ...
    Raises [Illegal] if [deck] does not have enough cards to deal out. *)
...
(** [get_player_hand pl] shows player [pl]'s hand. The hand will be sorted
...
(** [get_player_list st] shows the players in the game with state [st]. *)
...
(** [remove_card_top num st] returns a new game state with [num] cards removed
    from the deck of game state [st].

    Raises [NoCardsLeft] if the deck of [st] is empty. *)

val draw_from_pile : state -> player -> player
(** [remove_top_card deck] removes the top card of a non-empty [deck].

    Raises [NoCardsLeft] if [deck] is empty. *)

val get_current_player : state -> player
(** [get_current_player state] returns the id of the current player of the game
    with state [st].

    Raises [NoPlayer] if [state] does not have a current player. *)

val exchange_cards : player -> player -> int -> state -> state
(** [exchange_cards receiver sender card st] is the new state after cards of
    type [card] are moved from the [sender] to the [receiver] in state [st]. *)

val find_player : string -> player list -> player
(** [find_player name pl] checks if the player with name [name] is in the player
    list [pl].

    Raises [NoPlayer] if the player with name [name] is not in [pl]. *)

val get_deck : state -> int list
(** [get_deck st] returns the deck of state [st]. *)

val get_current_player_state : state -> int
(** [get_current_player_state st] returns the current player of the game of
    state [st]. *)

val assign_id : state -> int -> state
(** [assign_id st num] returns the state [st] after assigning each of the [num]
    number of players an id. *)
...
(** [repeat_add_card pl card num] returns [pl] after adding [card] [num] times
    to [pl]'s hand. *)

val delete_cards : int -> player -> player
(** [delete_cards card pl] returns [pl] after removing each instance of [card]
    from [pl]'s hand. *)

val check_quad : player -> int list
(** [check_quad pl] returns an empty list if the player [pl] does not have 4
    cards of the same value and a list with each of the card values that the
    player [pl] has 4 cards of. *)
...
(** [add_quad pl] returns the player [pl] after it is updated to have a higher
    ...
    Raises [Illegal] if [pl] does not have a quad. *)
...
(** [add_log st message] returns a game state [st] with [message] added to its
    logs. *)

val check_winner : state -> string list
(** [check_winner st] returns the winner(s) of the game with state [st]. *)

val set_score : player -> int -> player
(** [set_score pl score] sets the score of player [pl] to [score]. *)
